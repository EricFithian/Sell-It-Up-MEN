...
router.get('/', async (req, res, next) => {
    try { // fingers crossed 
        const products = await db.Product.find({}); // await is used when you have a promise you are waiting for
        ...
        db.Product.find({})
        .then(foundData=>{do something with your data res.send / res.render})
        .catch(err=>)
        // Product - then -> db.Reviews -> ad nauseum {{{{{{{}}}}}}}
        */
    } catch (error) {
        console.log(error);
        req.error = error;
